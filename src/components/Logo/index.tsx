...
export const Logo = () => {
    ...
    const handleLoadingAnimation = (value: boolean) => {
        setTimeout(() => {
    ...
    React.useEffect(() => {
        if (svgRef.current) {
            ...
            // force run one time only
            if (top && topRef.current === 0) {
                topRef.current = top
            }
            ...
            window.addEventListener('scroll', () => {
                //@ts-ignore
                const { top } = svgRef.current?.getBoundingClientRect()
                if ((topRef.current > 0 && top < topRef.current) || (topRef.current < 0 && top > topRef.current)) {
                    setWidth(36) // width for only letter "M"
    ...
    return (
        <span className="icon-svg-marvel-logo" aria-hidden="true">
            <LogoSvg
                ...
                height="52"
            >
                <rect fill="#EC1D24" width={width} height="100%"></rect>
                {!loadingAnimation ?
                    <path
                        fill="#FEFEFE"
